...
export default {
  ...
  methods: {
    ...
    async submitBook() {
      ...
      try {
        const AuthorRes = await fetch(`http://localhost:3000/api/authors?name=${encodeURIComponent(authorName)}`);
        const authorData = await AuthorRes.json();

        if (!authorData.found) {
          const createAuthorRes = await fetch(`http://localhost:3000/api/authors`, {
            method: 'POST',
      ...
      try {
        const genreRes = await fetch(`http://localhost:3000/api/genres?name=${encodeURIComponent(genreName)}`);
        const genreData = await genreRes.json();

        if (!genreData.found) {
          const createGenreRes = await fetch(`http://localhost:3000/api/genres`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
